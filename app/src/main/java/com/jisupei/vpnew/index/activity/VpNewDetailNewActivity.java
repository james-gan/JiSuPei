...
import com.jisupei.vpnew.cart.VpCartsActivity;
import com.jisupei.vpnew.db.VpNewProductDb;
import com.jisupei.vpnew.index.model.ProductDatilData;
import com.jisupei.vpnew.widget.AddNewVpNumCartPop;
 ...
 */
public class VpNewDetailNewActivity extends AppCompatActivity {
...
//    IndexProductList.IndexProductItem mIndexProductItem;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            mVpNewProductDbDaoDao = DatabaseHelper.getHelper(VpNewDetailNewActivity.this).getVpNewProductDbDaoDao();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        setContentView(R.layout.vp_new_detail);
        StatusBarUtil.setStatusBarColor(this,R.color.white);
        int a2=  StatusBarUtil.StatusBarLightMode(VpNewDetailNewActivity.this);
        if(a2==0){
            StatusBarUtil.setStatusBarColor(VpNewDetailNewActivity.this,R.color.black);
        }
...
//         mIndexProductItem= (IndexProductList.IndexProductItem) getIntent().getSerializableExtra("dish");
        ...
        //查看是不是收藏了
        LoadGetvpisFavoryProduct(productId);
          final int count=  queryCountNum();
        ...
        go_cart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(!HttpBase.islogin){
                    Intent intent =new Intent(VpNewDetailNewActivity.this, LoginActivity.class);
                    startActivity(intent);
                    finish();
                    return;
                }
                if("0".equals(cart_num.getText().toString())){
                    ToasAlert.toastCenter("购物车没有添加过商品");
                    return;
                }
                Intent intent = new Intent(VpNewDetailNewActivity.this,VpCartsActivity.class);
                startActivity(intent);
//                EventBus.getDefault().post("cartSelect");
        ...
        soucang.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if("Y".equals(isFavory)) {
                    AppLoading.show(VpNewDetailNewActivity.this);
                    HttpUtil.getInstance().getvpDelFavory(productId, new StringCallback() {
                        @Override
                        ...
                        public void onResponse(String response, int id) {
                            ...
                            if("yes".equals(optFlag)) {
                                ToasAlert.toastCenter("取消收藏商品成功！");
                                sc_iv.setImageResource(R.mipmap.sc);
                                isFavory="N";
                            }else{
                         ...
                         AppLoading.show(VpNewDetailNewActivity.this);
                            HttpUtil.getInstance().getVpAddFavory(productId, new StringCallback() {
                                @Override
                                public void onError(Call call, Exception e, int id) {
                                    AppLoading.close();
                                    ToasAlert.toastCenter("连接服务器失败，请稍后再试！");
                                ...
                                public void onResponse(String response, int id) {
                                    ...
                                    if("yes".equals(optFlag)) {
                                        ToasAlert.toastCenter("收藏商品成功！");
                                        sc_iv.setImageResource(R.mipmap.scselect);
                                        isFavory="Y";
                                    }else{
                                        ToasAlert.toastCenter(object.optString("optDesc"));
        ...
        add_cart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                AddNewVpNumCartPop mPopupWindow = new AddNewVpNumCartPop(VpNewDetailNewActivity.this,mDefaultContactShop);
               getWindow().setFlags(WindowManager.LayoutParams.FLAG_BLUR_BEHIND, WindowManager.LayoutParams.FLAG_BLUR_BEHIND);
                int[] location = new int[2];
                view.getLocationOnScreen(location);
                mPopupWindow.showAtLocation(add_cart,  Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
//				  mPopupWindow.showAsDropDown(right_listview);
                mPopupWindow.setOnRefresh(new AddNewVpNumCartPop.Refresh() {
                    @Override
                    public void refresh() {
//                        notifyDataSetChanged();
    ...
    ProductDatilData mDefaultContactShop;
  String   isFavory="N";
public  void LoadGetvpisFavoryProduct(String productId){//判断是不是收藏了
    if(!HttpBase.islogin){return;}

    AppLoading.show(VpNewDetailNewActivity.this);
    HttpUtil.getInstance().getvpisFavoryProduct(productId, new StringCallback() {
        @Override
        public void onError(Call call, Exception e, int id) {
            AppLoading.close();
            ToasAlert.toastCenter("连接服务器失败，请稍后再试！");
        ...
        public void onResponse(String response, int id) {
            ...
            if("yes".equals(optFlag)) {
                    ...
                    isFavory= object.optString("isFavory");
                if("Y".equals(isFavory)) {
                    sc_iv.setImageResource(R.mipmap.scselect);
                }else {
                    sc_iv.setImageResource(R.mipmap.sc);
    ...
    //查询商品的详情的接口
     public void productInfoData(String productid){
         ...
         HttpUtil.getInstance().vpnewProductInfo(productid, new StringCallback() {
             @Override
             public void onError(Call call, Exception e, int id) {
                 AppLoading.close();
                 ToasAlert.toastCenter("连接服务器失败，请稍后再试！");
...
//                     Button add_cart;
                     ...
                     if(mDefaultContactShop.imgList!=null &&mDefaultContactShop.imgList.size()>0){
                         for (ProductDatilData.Img mImg :mDefaultContactShop.imgList) {
                  ...
                  if(mDefaultContactShop.rkpriceList!=null && mDefaultContactShop.rkpriceList.size()>0){
                      ...
                      jiage_view2.setVisibility(View.VISIBLE);
                        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(VpNewDetailNewActivity.this);
                        linearLayoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);
                        jiage_list.setLayoutManager(linearLayoutManager);
                        QujianAdapter adapter = new QujianAdapter(VpNewDetailNewActivity.this,mDefaultContactShop.rkpriceList,mDefaultContactShop.unit);
...
//         auto_view_pager.setCurrentItem((Integer.MAX_VALUE / 2 - Integer.MAX_VALUE / 2 % imageList.size()));
        ...
        for (int i = 0; i < imageList.size(); i++) {
            // 添加指示点
            ImageView point = new ImageView(VpNewDetailNewActivity.this);
...
//            draweeView.setImageURI(uri);
            ...
            ImageView draweeView= new ImageView(VpNewDetailNewActivity.this);
            draweeView.setScaleType( ImageView.ScaleType.FIT_XY);
            Picasso.with(VpNewDetailNewActivity.this).load(HttpUtil.HOST_STRING +"/"+ mSimpleDraweeViewList.get(position % mSimpleDraweeViewList.size())).error(R.mipmap.error).into(draweeView);
...
//            List<Product> mProductList  =cartDao.queryBuilder().where().eq("id", id).and().eq("account",HttpBase.getAccount(getApplicationContext())).query();
    ...
    public class QujianAdapter extends  RecyclerView.Adapter<QujianAdapter.MyViewHolder> {
        Context context;
        String unit;
        public List<ProductDatilData.RkpriceListItem> rkpriceList;
