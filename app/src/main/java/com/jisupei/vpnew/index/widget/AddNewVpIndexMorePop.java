...
public class AddNewVpIndexMorePop extends PopupWindow {


    private View view;

    private Context mContext;
    IndexData.ProductNew  mProductItem;
...
//    EditText dai_num;
  TextView now_price;
    ...
    public AddNewVpIndexMorePop(final Activity context, IndexData.ProductNew  mProductItem) {
        super(context);
...
//        TextView dai_danwei = (TextView)view.findViewById(R.id.dai_danwei);
        ...
        if(bean!=null){//是购物车有数据
            ...
            if(mProductItem.rkpriceList!=null && mProductItem.rkpriceListArr.size()>0){//有区间
                String  nowCurrPrice =   getInsertPrice(TextUtils.isEmpty(jianNumDb+"")?"0":(jianNumDb+""),mProductItem.rkpriceListArr);
...
//价格end
        ...
        //区间列表
        if(mProductItem.rkpriceList!=null && mProductItem.rkpriceListArr.size()>0){
...
//            List<VpNewProductDb.RkpriceListItemDb>  rkpriceList = new Gson().fromJson(bean.rkpriceListJson, new TypeToken<ArrayList<VpNewProductDb.RkpriceListItemDb>>(){}.getType());
            QujianAdapter adapter = new QujianAdapter(mContext,mProductItem.rkpriceListArr,mProductItem.unit);
...
//                }
                double chen=       HttpBase.mul(Double.parseDouble(TextUtils.isEmpty(jian.getText().toString().trim())?"0":jian.getText().toString()),Double.parseDouble(mProductItem.equation_factor) );
                  //
                if(mProductItem.rkpriceList!=null && mProductItem.rkpriceListArr.size()>0){
                        String  nowCurrPrice =   getInsertPrice(TextUtils.isEmpty(jian.getText().toString())?"0":jian.getText().toString(),mProductItem.rkpriceListArr);
                        if(!TextUtils.isEmpty(nowCurrPrice)){
                            now_price.setText("￥" + nowCurrPrice+"/"+mProductItem.unit);
                        }else {
                            now_price.setText("￥" + mProductItem.unit_price+"/"+mProductItem.unit);
    ...
    public class QujianAdapter extends  RecyclerView.Adapter<QujianAdapter.MyViewHolder> {
        Context context;
        String unit;
        List<IndexData.RkpriceListItem>  rkpriceList;

        public QujianAdapter(Context context , List<IndexData.RkpriceListItem> rkpriceList, String unit) {
        ...
        public void onBindViewHolder(final MyViewHolder holder, int position) {
             final IndexData.RkpriceListItem mRkpriceListItem   =  rkpriceList.get(position);
...
//            List<Product> mProductList  =cartDao.queryBuilder().where().eq("id", id).and().eq("account", HttpBase.getAccount(mContext)).query();
    ...
    //首页，添加修改购物车数据
    private void updateNew(String idNew, IndexData.ProductNew mProductItem) {
        try {
...
//                    }
    ...
    //首页添加购物车
    public void  addCartNew(String jian1,IndexData.ProductNew mProductItem){
        try {
...
//            bean.setAccount(HttpBase.getAccount(mContext));
           ...
           if(mProductItem.rkpriceListArr!=null && mProductItem.rkpriceListArr.size()>0){
              Gson gson  =new Gson();
    ...
    //根据数量获得区间的价格
    public String getInsertPrice(String num,List<IndexData.RkpriceListItem>  rkpriceList){
        num=TextUtils.isEmpty(num)?"0":num;
        String curr="";
        for(IndexData.RkpriceListItem mRkpriceListItem:rkpriceList){
            double  numDouble =Double.parseDouble(num);
